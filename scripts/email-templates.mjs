// Templates emails HTML brandés KYRAN
// Design épuré, responsive, inline CSS compatible webmails (Gmail, Apple Mail, Outlook).
...
<body style="margin: 0; padding: 0; background-color: #f1f5f9; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased; color: #0f172a;">
  <!-- Prévisualisation boîte de réception -->
    ...
    <tr>
      <td align="center">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(15, 23, 42, 0.06); border: 1px solid #e2e8f0;">
          ...
          <!-- En-tête KYRAN avec accent doré -->
          <tr>
          ...
          <tr>
            <td style="background-color: #f8fafc; border-top: 1px solid #e2e8f0; padding: 24px 32px; text-align: center;">
              ...
              <p style="margin: 0 0 16px 0; font-size: 12px; color: #94a3b8;">
                Une question sur votre commande ? Répondez directement à cet email ou écrivez à <a href="mailto:contact@kyran-jeu.fr" style="color: #d97706; text-decoration: underline;">contact@kyran-jeu.fr</a>
              </p>
...
export function renderOrderEmail({
  ...
  const contentHtml = `
    ...
    <div style="background-color: #eff6ff; border: 1px solid #bfdbfe; border-radius: 10px; padding: 14px 18px; margin-top: 20px; font-size: 13px; line-height: 1.5; color: #1e40af;">
      ...
      <div style="margin-top: 4px;">Délai d'acheminement estimé : <strong>${estimatedDelivery}</strong> via La Poste / Colissimo suivi. Vous recevrez un email dès l'expédition avec votre numéro de suivi.</div>
    </div>

    <!-- Encadré Dojo / En attendant -->
    ...
    ${orderId ? `<div style="text-align: center; margin-top: 20px; font-size: 11px; color: #94a3b8;">Référence Stripe : ${orderId}</div>` : ''}
  `;

  return baseLayout({
    title: 'Confirmation de commande KYRAN',
    previewText: `Merci ${customerName} ! Votre boîte de jeu KYRAN est en cours de préparation.`,
    contentHtml
  });
}
...
export function renderShippingEmail({
  ...
  const contentHtml = `
    ...
    <div style="text-align: center; margin-bottom: 26px;">
      ...
      <p style="margin: 0; font-size: 14px; color: #64748b;">
        Votre commande a été remise au transporteur et arrive très bientôt.
      </p>
    </div>
    ...
    ${orderId ? `<div style="text-align: center; margin-top: 22px; font-size: 11px; color: #94a3b8;">Référence commande : ${orderId}</div>` : ''}
  `;

  return baseLayout({
    title: 'Votre jeu KYRAN a été expédié !',
    previewText: `Bonne nouvelle ${customerName} ! Votre colis KYRAN est en route (${carrier} - ${trackingNumber}).`,
    contentHtml
  });
}

/**
 * 3. Email de confirmation de remboursement
 */
export function renderRefundEmail({
  ...
  const contentHtml = `
    ...
    <div style="background-color: #eff6ff; border: 1px solid #bfdbfe; border-radius: 10px; padding: 14px 18px; font-size: 13px; line-height: 1.5; color: #1e40af;">
      ...
      Selon votre établissement bancaire, le montant apparaîtra sur votre compte bancaire sous <strong>5 à 10 jours ouvrés</strong> (sur la carte utilisée lors du paiement Stripe).
    </div>

    <p style="font-size: 13px; color: #64748b; line-height: 1.6; margin-top: 24px; text-align: center;">
